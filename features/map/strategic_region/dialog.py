...
class StrategicRegionDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        new_btn = QPushButton("新建空 Region")
        ...
        weather_row.addWidget(QLabel("天气预设:"))
        ...
        # 省份数
        self._prov_count_label = QLabel("省份: 0")
        right.addWidget(self._prov_count_label)

        # 拾取按钮
        self._pick_btn = QPushButton("开始拾取省份")
    ...
    def _on_select(self, item: QListWidgetItem) -> None:
        ...
        self._prov_count_label.setText(f"省份: {len(r.province_ids)}")

    # ─────────── 操作 ───────────

    def _on_auto_generate(self) -> None:
        if self._province_map is None or self._tile_map is None:
            QMessageBox.warning(self, "错误", "需要先生成省份")
            return
        ret = QMessageBox.question(
            self, "自动生成",
            "将按当前 State 分组自动生成战略区域.\n"
            "weather 按纬度自动分配.\n\n"
            "⚠ 这会覆盖已有的战略区域数据.\n\n继续?",
        )
        ...
        self._refresh_list()
        self._status.setText(f"已生成 {self._mgr.count()} 个区域")
    ...
    def _on_pick_toggled(self, on: bool) -> None:
        ...
        if on and rid <= 0:
            ...
            QMessageBox.warning(self, "错误", "请先选中一个 Region")
            return
        self._pick_on = on
        if on:
            self._pick_btn.setText("停止拾取")
            self._status.setText(f"点击主画布省份 → 加入 Region #{rid}")
        else:
            self._pick_btn.setText("开始拾取省份")
    ...
    def notify_assigned(self, pid: int) -> None:
        ...
        if r:
            self._prov_count_label.setText(f"省份: {len(r.province_ids)}")
            self._status.setText(f"已指派省份 {pid} → Region #{rid}")
        self._refresh_list()
